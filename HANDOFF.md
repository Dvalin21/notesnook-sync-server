...
## 2026-09-06 user-verified (tablet + phone)
- Cross-device note sync: WORKS
- Monograph publish/view: WORKS
- Image attachment render across logout/login: WORKS (earlier failure was
  attachment-insert vs image-insert usage, not server)
- Remaining untested: account delete, email change, recovery mail, recovery
  codes, authenticator MFA, multipart uploads, inbox E2E, quota, refresh>1h
- Known gaps (code): password change disabled, ClearSessionsAsync
  NotImplemented, SSE push dead (WAMP removed), silent MFA queue, unbounded
  WAMP retry (hardening parked until user gives a window)

## Next work (2026-09-06, after user verification round)
Status: GitHub master=e774a73 and Hub :latest (sync 3d73dfa, identity
26d7f06, sse bb61928) all match what is RUNNING. Nothing pending to push.

1. Hardening deploy (parked, needs 2-min quiet window + rebuild):
   - Bound WampHelper.OpenWampChannelAsync retry (infinite 5s loop today —
     any dead WAMP endpoint hangs the request; caused the 92s signup 504
     via subscription publish when SELF_HOSTED was 0).
   - MFA super_strict QueueLimit int.MaxValue -> small (fail fast 429
     instead of silent queue).
   - Confirm-callback SSE publish: best-effort (SSE has no WAMP listener).
2. Password change: PATCH /users/password/* disabled + HTTP path
   NotImplemented — implement or keep disabled deliberately.
3. ClearSessionsAsync NotImplemented (logout-everywhere fails).
4. Inbox E2E: needs MX routing for inbox domain + test mail.
5. Untested: account delete, email change, recovery mail/codes,
   authenticator MFA, multipart uploads, quota, refresh>1h.
6. Watch: SELF_HOSTED must stay 1 in .env (0 enables cloud paths that
   hang). NEVER down -v on live (wipes mongo+keystore+minio).
7. Consider: GitHub history still contains HANDOFF.md + test email in old
   commits (untracked going forward since e774a73; scrub history only if
   repo must be fully clean).

...
- **Branch**: `master`
- **Domain**: example.com (updated from example.com)
...
curl -X POST -H "Host: auth.example.com" \
  "http://localhost:8080/connect/token" \
  -d "grant_type=email&email=test@example.com&client_id=notesnook"
...
## DNS required
`*.example.com` wildcard A record covering: sync, auth, sse, notes, attach, minio, cors, inbox, themes, plus apex.
